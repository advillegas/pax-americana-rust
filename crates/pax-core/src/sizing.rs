...
///
/// Returns `None` if balances are unknown (caller must skip trading). The result is the
/// master net scaled by the balance ratio and multiplier, rounded to whole shares. `price`
/// is unused (kept for call-site compatibility / future use).
pub fn target_net_qty(master_net: f64, _price: f64, p: &SizingParams) -> Option<f64> {
    let ratio = p.ratio()?;
